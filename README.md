...
PDF 页码与书页号**，方便论文写作时核对引文原始出处。

## 核心思路

```
PDF ──▶ 逐页高清渲染 ──▶ Apple Vision OCR(本地免费) ──▶ 版面分析
(300dpi)                   (中英混排/竖排)                (页眉/页脚/页码/脚注)
                                                              │
                          ┌───────────────────────────────────┤
                          ▼                                   ▼
              段落组装(跨页续段/标题层级/脚注)         PDF页 ↔ 书页 映射表
              (可选 DeepSeek 精修标题与元数据)         (解决单双页错位)
                          │
                          ▼
               Markdown(带页码注释) ──▶ pandoc ──▶ Word(.docx)
```

- **OCR 引擎**：macOS 自带 Vision 框架（免费、离线、M3 神经引擎加速），
  支持简体/繁体中文、英文混排。
- **页码锚定**：以 PDF 页为准（满足"书页单双页错位"场景），同时从页眉页脚
  识别书页号，生成映射表。
...
#   --inline-pages  每个正文段落后追加〔PDF第N页〕标记（Word 中可见）
#   --no-footnotes  丢弃脚注
...
| 文件 | 说明 |
|---|---|
| `输出目录/书名.md` | 带页码注释的 Markdown（正文+标题+脚注） |
| `输出目录/书名.docx` | Word 版（标题样式、目录；页码注释对 Word 隐藏） |
| `输出目录/page_map.csv` | **PDF页 ↔ 书页** 映射表 |
| `输出目录/paragraphs.jsonl` | 每个段落的来源页码索引（程序化检索用） |
| `输出目录/meta.json` | 元数据与统计 |

Markdown 中的页码标记（Word 里自动隐藏）：
`<!-- ⏸ PDF 第 12 页（书页 34） -->`
