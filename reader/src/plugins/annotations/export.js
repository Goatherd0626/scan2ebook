...
export async function buildAnnotatedS2e(book, records) {
  const zip = new JSZip();
  zip.file('book.pdf', await book.pdfBlob.arrayBuffer());
...
export async function downloadAnnotatedS2e(book, records) {
  const blob = await buildAnnotatedS2e(book, records);
